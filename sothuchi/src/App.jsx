...
// ─── SAMPLE DATA ─────────────────────────────────────────────────────────────
const NOW_M = toMonth();
const NOW_Y = NOW_M.slice(0,4);
const PREV_M = `${NOW_Y}-${String(parseInt(NOW_M.slice(5))-1).padStart(2,"0")}`;

const INIT_TXN = [
  { id:1,  date:`${NOW_M}-15`,  amount:65000,    type:"expense", category:"food",        note:"Ăn trưa phở bò" },
  { id:2,  date:`${NOW_M}-15`,  amount:45000,    type:"expense", category:"travel",      note:"Grab đi làm" },
  { id:3,  date:`${NOW_M}-14`,  amount:12000000, type:"income",  category:"salary",      note:"Lương tháng này" },
  { id:4,  date:`${NOW_M}-14`,  amount:350000,   type:"expense", category:"daily",       note:"Siêu thị VinMart" },
  { id:5,  date:`${NOW_M}-13`,  amount:220000,   type:"expense", category:"cosmetics",   note:"Son môi MAC" },
  { id:6,  date:`${NOW_M}-12`,  amount:150000,   type:"expense", category:"phone",       note:"Nạp Viettel" },
  { id:7,  date:`${NOW_M}-10`,  amount:800000,   type:"expense", category:"clothes",     note:"Áo thun H&M" },
  { id:8,  date:`${NOW_M}-08`,  amount:540000,   type:"expense", category:"health",      note:"Khám bệnh viện 108" },
  { id:9,  date:`${NOW_M}-05`,  amount:600000,   type:"expense", category:"electricity", note:"Tiền điện tháng này" },
  { id:10, date:`${NOW_M}-03`,  amount:135000,   type:"expense", category:"food",        note:"Highlands Coffee" },
  { id:11, date:`${NOW_M}-02`,  amount:500000,   type:"income",  category:"bonus",       note:"Thưởng hoàn thành dự án" },
  { id:12, date:`${NOW_M}-01`,  amount:3500000,  type:"expense", category:"rent",        note:"Tiền nhà tháng này" },
  { id:13, date:`${PREV_M}-28`, amount:280000,   type:"expense", category:"social",      note:"Nhậu với bạn bè" },
  { id:14, date:`${PREV_M}-25`, amount:11500000, type:"income",  category:"salary",      note:"Lương tháng trước" },
  { id:15, date:`${PREV_M}-20`, amount:450000,   type:"expense", category:"daily",       note:"Đi chợ cuối tuần" },
  { id:16, date:`${PREV_M}-15`, amount:75000,    type:"expense", category:"food",        note:"Bún bò Huế" },
  { id:17, date:`${PREV_M}-10`, amount:1200000,  type:"expense", category:"education",   note:"Khoá học Udemy" },
];
...
const INIT_DEBTS = [
  { id:1, name:"Nguyễn Văn A",     amount:500000,  remaining:500000,  dueDate:`${NOW_Y}-06-01`, type:"lent", note:"Cho mượn tiền ăn" },
  { id:2, name:"Thẻ tín dụng VCB", amount:2000000, remaining:1500000, dueDate:`${NOW_M}-25`,     type:"owed", note:"Trả thẻ tháng này" },
];

// ─── AI AUTO-CATEGORISE ──────────────────────────────────────────────────────
const CAT_KW = {
  food:        ["ăn","cơm","phở","bún","café","cafe","cà phê","trà","bánh","pizza","lẩu","nướng","nhậu","quán","burger","sushi","gà"],
  travel:      ["grab","taxi","xăng","xe","bus","gojek","be","xe ôm","xăng dầu","vé","máy bay","tàu"],
  clothes:     ["quần","áo","giày","dép","thời trang","h&m","zara","uniqlo","váy","túi","mũ"],
  health:      ["thuốc","bác sĩ","khám","bệnh viện","y tế","pharmacy","nhà thuốc","bệnh","vitamin","gym"],
  education:   ["học","sách","khóa","học phí","trường","udemy","coursera","tiếng anh","luyện thi"],
  electricity: ["điện","nước","gas","internet","wifi","điện nước","mobi","viettel wifi"],
  cosmetics:   ["mỹ phẩm","kem","son","nước hoa","mac","innisfree","skincare","serum","toner"],
  social:      ["tiệc","giao lưu","sinh nhật","party","nhậu","bạn bè","karaoke","event"],
  phone:       ["điện thoại","viettel","vinaphone","mobifone","nạp tiền","sim","data","4g"],
  rent:        ["nhà","phòng trọ","thuê","chung cư","tiền nhà","phòng"],
};
...
export default function App() {
  ...
  const [transactions, setTxn]       = useState(INIT_TXN);
  ...
  // AI chat
  const [aiMsgs, setAiMsgs] = useState([{role:"assistant",content:"👋 Xin chào! Tôi là trợ lý tài chính AI.\n\nHỏi tôi bất cứ điều gì về thu chi, tiết kiệm hay kế hoạch tài chính nhé! 💰"}]);
  ...
  // ── computed ────────────────────────────────────────────────────────────
  const monthTxn = useMemo(() => transactions.filter(t => t.date.startsWith(month)), [transactions, month]);
  const totalInc = useMemo(() => monthTxn.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0), [monthTxn]);
  const totalExp = useMemo(() => monthTxn.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0), [monthTxn]);
  const balance  = useMemo(() => transactions.reduce((s,t)=>t.type==="income"?s+t.amount:s-t.amount,0), [transactions]);

  const catData = useMemo(() => {
    const m = {};
    monthTxn.filter(t=>t.type==="expense").forEach(t=>{m[t.category]=(m[t.category]||0)+t.amount;});
  ...
  const alerts = useMemo(()=>{
    ...
    debts.forEach(d=>{
      ...
      if(diff>=0&&diff<=7&&d.type==="owed")a.push(`🔔 Trả nợ ${d.name}: ${fmtS(d.remaining)} (${diff}ngày)`);
    });
    return a;
  }, [budgetData,debts]);
  ...
  const filteredTxn = useMemo(()=>{
    ...
    return list;
  }, [monthTxn, search, filterCat]);

  const grouped = useMemo(()=>{
    const g={};
    filteredTxn.forEach(t=>{(g[t.date]=g[t.date]||[]).push(t);});
  ...
  const trendData = useMemo(()=>{
    return Array.from({length:6},(_,i)=>{
      ...
      return {
        ...
        income:txns.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0)/1e6,
        expense:txns.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0)/1e6,
      };
    });
  }, [transactions]);
  ...
  const handleAdd = () => {
    const amt=parseInt(iAmt.replace(/[^0-9]/g,""));
    if(!amt){showToast("Vui lòng nhập số tiền","error");return;}
    const newTxn={
      id:Date.now(),date:iDate,amount:amt,type:iType,
      category:iType==="expense"?iCat:"salary",
      note:iNote||(iType==="expense"?"Chi tiêu":"Thu nhập"),
    };
    setTxn(prev=>[newTxn,...prev]);
    setIAmt("");setINote("");
    showToast("✅ Đã ghi thành công!");
  };

  const handleDel = id => {setTxn(prev=>prev.filter(t=>t.id!==id));setDelId(null);showToast("🗑️ Đã xóa giao dịch");};
  ...
  const handleAIChat = async () => {
    ...
    setAiLoad(true);
    const ctx=`Tháng ${month}: Thu=${fmt(totalInc)}, Chi=${fmt(totalExp)}, Số dư=${fmt(balance)}, Tỷ lệ tiết kiệm=${savingsRate}%. Chi: ${catData.map(c=>`${c.name} ${fmtS(c.value)}(${c.pct}%)`).join(", ")}. NS: ${budgetData.map(b=>`${b.cat.name} ${fmtS(b.spent)}/${fmtS(b.limit)}`).join(", ")}. Nợ: ${debts.map(d=>`${d.name} ${fmtS(d.remaining)} ${d.type==="owed"?"(trả)":"(đòi)"}`).join(", ")}. GD gần đây: ${transactions.slice(0,8).map(t=>`${t.date} ${t.type==="expense"?"-":"+"}${fmtS(t.amount)} ${t.note}`).join("; ")}.`;
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          model:"claude-sonnet-4-20250514",max_tokens:800,
          system:`Bạn là trợ lý tài chính AI trong ứng dụng Sổ Thu Chi Việt Nam. Trả lời tiếng Việt, ngắn gọn (≤150 từ), thân thiện, dùng emoji phù hợp. Đưa lời khuyên cụ thể và thực tế. Dữ liệu người dùng: ${ctx}`,
  ...
  const s = {
    ...
    hdr:     {background:`linear-gradient(145deg,${P},${PD})`,color:"#fff",padding:"20px 20px 24px",borderRadius:"0 0 28px 28px",boxShadow:"0 4px 20px rgba(249,115,22,.35)"},
    ...
    <div style={s.wrap}>
      ...
      {toast&&(
        <div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:999,
          background:toast.type==="error"?"#FEE2E2":"#D1FAE5",color:toast.type==="error"?"#991B1B":"#065F46",
          borderRadius:14,padding:"10px 20px",fontWeight:600,fontSize:13,
          boxShadow:"0 4px 20px rgba(0,0,0,.15)",whiteSpace:"nowrap",animation:"slideDown .3s ease"}}>
          {toast.msg}
        </div>
      )}
      ...
      {tab==="input"&&(
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{padding:16}}>
            {/* Savings rate card */}
            {totalInc>0&&(
              <div style={{...s.card,padding:"12px 16px",background:savingsRate>=20?`linear-gradient(135deg,#D1FAE5,#A7F3D0)`:`linear-gradient(135deg,#FEE2E2,#FCA5A5)`}}>
            ...
            <div style={s.card}>
              ...
              <div style={{display:"flex",gap:6,marginBottom:16,background:"#F1F5F9",borderRadius:14,padding:4}}>
                <button style={s.typeBtn(iType==="expense")} onClick={()=>{setIType("expense");setICat("food");}}>🔴 Chi tiêu</button>
                <button style={s.typeBtn(iType==="income")}  onClick={()=>{setIType("income");setICat("salary");}}>🟢 Thu nhập</button>
              </div>

              {/* Amount */}
              <div style={{marginBottom:12}}>
                <label style={{fontSize:12,color:"#94A3B8",fontWeight:600,letterSpacing:.5,display:"block",marginBottom:5}}>SỐ TIỀN</label>
                <input style={{...s.inp,fontSize:26,fontWeight:800,color:iType==="expense"?"#EF4444":"#10B981",textAlign:"right",letterSpacing:-1}}
              ...
              <div style={{marginBottom:12}}>
                ...
                <input type="date" style={s.inp} value={iDate} onChange={e=>setIDate(e.target.value)}/>
              </div>
              ...
              <div style={{marginBottom:14}}>
                ...
                <input style={s.inp} placeholder='VD: "Ăn phở bò", "Grab đi làm", "Lương tháng 5"…'
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {EXPENSE_CATS.map(cat=>(
                      <button key={cat.id} style={s.catBtn(iCat===cat.id)} onClick={()=>setICat(cat.id)}>
      ...
      {tab==="list"&&(
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{padding:"10px 16px 0"}}>
            <input style={{...s.inp,marginBottom:8}} placeholder="🔍 Tìm giao dịch…" value={search} onChange={e=>setSearch(e.target.value)}/>
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:6}}>
              {[{id:"all",name:"Tất cả",icon:"📋"},...EXPENSE_CATS.slice(0,6)].map(c=>(
                <button key={c.id} onClick={()=>setFilterCat(c.id)} style={{
                  flexShrink:0,fontSize:11,padding:"4px 10px",borderRadius:20,
                  border:`1.5px solid ${filterCat===c.id?P:"#E2E8F0"}`,
                  background:filterCat===c.id?PL:"#fff",color:filterCat===c.id?PD:"#64748B",
          ...
          <div style={{padding:"0 16px 12px"}}>
            {grouped.length===0?(
              <div style={{textAlign:"center",padding:"60px 20px",color:"#94A3B8"}}>
                ...
                <p style={{fontSize:15,marginTop:8}}>{search?"Không tìm thấy kết quả":"Chưa có giao dịch tháng này"}</p>
                {!search&&<button style={{...s.btn(P),width:"auto",padding:"10px 24px",marginTop:8}} onClick={()=>setTab("input")}>+ Thêm giao dịch</button>}
              </div>
            ):grouped.map(([date,txns])=>{
              const dayTotal=txns.reduce((sum,t)=>t.type==="expense"?sum-t.amount:sum+t.amount,0);
      ...
      {tab==="report"&&(
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{padding:"12px 16px"}}>
            {/* Savings rate */}
            {totalInc>0&&(
              <div style={{...s.card,background:savingsRate>=20?"#F0FDF4":"#FFF1F2"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                ...
                <AreaChart data={trendData} margin={{top:0,right:0,left:-20,bottom:0}}>
                  <defs>
                    ...
                    <linearGradient id="ge" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={P} stopOpacity={.3}/>
            ...
            {catData.length>=1&&(
              <div style={{...s.card,background:"#FFF7ED",border:"1px solid #FED7AA"}}>
                <p style={{margin:"0 0 8px",fontSize:13,fontWeight:700,color:PD}}>💡 Phân tích nhanh</p>
                {catData.slice(0,3).map((c,i)=>(
                  <p key={c.id} style={{margin:"0 0 4px",fontSize:12,color:"#92400E"}}>
                    {i===0?"🔴":i===1?"🟠":"🟡"} {c.name} chiếm <b>{c.pct}%</b> chi tiêu tháng này ({fmtS(c.value)})
                  </p>
                ))}
                {savingsRate<20&&<p style={{margin:"8px 0 0",fontSize:12,color:"#991B1B",fontWeight:600}}>⚠️ Tỷ lệ tiết kiệm đang thấp hơn mục tiêu 20%</p>}
              </div>
            )}
          </div>
        </div>
      ...
      {tab==="budget"&&(
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{padding:"12px 16px"}}>
            {budgetData.map(b=>(
              <div key={b.category} style={{...s.card,borderLeft:`4px solid ${b.over?"#EF4444":b.pct>70?"#F59E0B":"#10B981"}`}}>
            ...
            <button style={s.btn()} onClick={()=>setBModal(true)}>+ Thêm / sửa ngân sách</button>
              ...
              <div style={{...s.card,marginTop:4}}>
                ...
                {catData.filter(c=>!budgets.find(b=>b.category===c.id)).map(c=>(
                  <div key={c.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:"1px solid #F1F5F9"}}>
                    ...
                    <button onClick={()=>{setBCat(c.id);setBModal(true);}} style={{fontSize:11,padding:"3px 10px",borderRadius:10,border:`1px solid ${P}`,background:PL,color:PD,cursor:"pointer",fontFamily:"inherit"}}>+ NS</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={s.card}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                ...
                <div>
                  ...
                  <p style={{margin:0,fontSize:11,color:"#10B981",fontWeight:600}}>● Claude Sonnet</p>
                </div>
              </div>
              ...
              <div ref={chatRef} style={{height:280,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,marginBottom:12,padding:"4px 0"}}>
                {aiMsgs.map((m,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",gap:8,alignItems:"flex-end"}}>
                    ...
                    <div style={{maxWidth:"82%",padding:"10px 14px",
                      ...
                      background:m.role==="user"?`linear-gradient(135deg,${P},${PD})`:"#F1F5F9",
                      color:m.role==="user"?"#fff":"#1E293B",fontSize:13,lineHeight:1.6,whiteSpace:"pre-wrap"}}>
                      {m.content}
                    </div>
                  </div>
                ))}
                {aiLoad&&(
                  <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
                    ...
                    <div style={{background:"#F1F5F9",padding:"12px 16px",borderRadius:"18px 18px 18px 4px",fontSize:13,color:"#64748B"}}>
                      ⏳ Đang phân tích…
                    </div>
                  </div>
                )}
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                {["Tháng này chi nhiều nhất đâu?","Lời khuyên tiết kiệm","Dự báo cuối tháng","Tôi có đang tiêu quá không?"].map(q=>(
                  <button key={q} onClick={()=>setAiIn(q)} style={{fontSize:11,padding:"4px 10px",borderRadius:20,border:`1px solid ${PM}`,background:PL,color:PD,cursor:"pointer",fontFamily:"inherit"}}>
                    {q}
                  </button>
                ))}
              </div>
              ...
              <div style={{display:"flex",gap:8}}>
                <input style={{...s.inp,flex:1,borderRadius:30,padding:"12px 16px"}}
                  placeholder="Hỏi về tài chính của bạn…"
                  value={aiIn} onChange={e=>setAiIn(e.target.value)}
                  onKeyDown={e=>e.key==="Enter"&&handleAIChat()}/>
                <button onClick={handleAIChat} disabled={aiLoad} style={{
                  background:aiLoad?"#CBD5E1":`linear-gradient(135deg,${P},${PD})`,
                  border:"none",color:"#fff",borderRadius:30,padding:"0 20px",cursor:aiLoad?"default":"pointer",fontSize:20,flexShrink:0}}>↑</button>
              </div>
            </div>
              ...
              <div style={{display:"flex",alignItems:"center",marginBottom:14}}>
                <div>
                  ...
                  <p style={{margin:0,fontSize:11,color:"#94A3B8"}}>
                    Cần trả: {fmtS(debts.filter(d=>d.type==="owed").reduce((s,d)=>s+d.remaining,0))} ·
                    Cần đòi: {fmtS(debts.filter(d=>d.type==="lent").reduce((s,d)=>s+d.remaining,0))}
                  </p>
                </div>
                ...
                return (
                  <div key={d.id} style={{padding:"12px 0",borderBottom:"1px solid #F1F5F9"}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:8}}>
                      <div style={{width:44,height:44,borderRadius:14,background:d.type==="owed"?"#FEE2E2":"#D1FAE5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>
                        {d.type==="owed"?"🔴":"🟢"}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      {d.remaining>0&&(
                        ...
                        }} style={{fontSize:11,padding:"4px 10px",borderRadius:10,border:`1px solid ${P}`,background:PL,color:PD,cursor:"pointer",fontWeight:600,fontFamily:"inherit"}}>
                          Thanh toán
                        </button>
                      )}
                    </div>
      ...
      {bModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"flex-end",zIndex:200,backdropFilter:"blur(4px)"}}>
          <div style={{background:"#fff",borderRadius:"28px 28px 0 0",padding:24,width:"100%",maxWidth:480,margin:"0 auto",boxShadow:"0 -8px 40px rgba(0,0,0,.2)"}}>
            ...
            <select style={{...s.inp,marginBottom:12}} value={bCat} onChange={e=>setBCat(e.target.value)}>
            ...
            <input style={{...s.inp,marginBottom:20,fontSize:20,fontWeight:700}} placeholder="VD: 2,000,000" value={bAmt} onChange={e=>handleAmt(e.target.value)} inputMode="numeric"/>
            <div style={{display:"flex",gap:10}}>
              <button style={{...s.btn("#F1F5F9","#475569"),flex:1}} onClick={()=>setBModal(false)}>Hủy</button>
              <button style={{...s.btn(),flex:1}} onClick={saveBudget}>Lưu</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"flex-end",zIndex:200,backdropFilter:"blur(4px)"}}>
          <div style={{background:"#fff",borderRadius:"28px 28px 0 0",padding:24,width:"100%",maxWidth:480,margin:"0 auto",boxShadow:"0 -8px 40px rgba(0,0,0,.2)"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14,background:"#F1F5F9",borderRadius:14,padding:4}}>
              <button style={s.typeBtn(dForm.type==="owed")}  onClick={()=>setDForm(f=>({...f,type:"owed"}))}>🔴 Tôi nợ</button>
              <button style={s.typeBtn(dForm.type==="lent")} onClick={()=>setDForm(f=>({...f,type:"lent"}))}>🟢 Người khác nợ tôi</button>
            </div>
            {[["Tên người / tổ chức","name","text"],["Số tiền","amount","numeric"],["Ngày đến hạn","dueDate","date"],["Ghi chú","note","text"]].map(([label,key,mode])=>(
              <div key={key} style={{marginBottom:10}}>
                ...
                <input type={mode==="date"?"date":"text"} style={s.inp} placeholder={label}
                  value={dForm[key]} inputMode={mode} onChange={e=>setDForm(f=>({...f,[key]:e.target.value}))}/>
              </div>
            ))}
            <div style={{display:"flex",gap:10,marginTop:6}}>
              <button style={{...s.btn("#F1F5F9","#475569"),flex:1}} onClick={()=>setDModal(false)}>Hủy</button>
              <button style={{...s.btn(),flex:1}} onClick={saveDebt}>Lưu</button>
            </div>
          </div>
        </div>
      )}
      ...
      {delId&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,backdropFilter:"blur(4px)"}}>
          <div style={{background:"#fff",borderRadius:24,padding:24,width:300,textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button style={{...s.btn("#F1F5F9","#475569"),flex:1}} onClick={()=>setDelId(null)}>Hủy</button>
              <button style={{...s.btn("#EF4444"),flex:1}} onClick={()=>handleDel(delId)}>Xóa</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{display:"flex",borderTop:"1px solid #F1F5F9",position:"sticky",bottom:0,zIndex:50,background:"#fff",boxShadow:"0 -4px 20px rgba(0,0,0,.06)"}}>
        {TABS.map(t=>(
          <div key={t.id} style={s.tabBtn(tab===t.id)} onClick={()=>setTab(t.id)}>
